...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        maintainer_group.permissions.set(
            permissions.filter(codename__in=[
                "view_uploadedfile",
                "add_uploadedfile",
                "change_uploadedfile",
            ])
        ...
        if token_file_name:
            ...
            token_file = os.path.join(BASE_DIR, "browser_token", )

            browser_user, created = User.objects.get_or_create(username=browser_username)
            if created:
                browser_user.set_password(browser_password)
                browser_user.save()
                browser_user.groups.add(client_group)
                self.stdout.write(self.style.SUCCESS(f'User "{browser_username}" created and asigned to client group.'))
